...
hist(daily_steps$steps, xlab = "Total number of steps by date")
```
...
```{r echo=TRUE}
## Here a simple_impute function is created that takes a set that needs to be imputed and a set with averages per 5-minute interval that are used to imput the NAs in the former.
